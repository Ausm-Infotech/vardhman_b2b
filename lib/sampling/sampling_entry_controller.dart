...
class SamplingEntryController extends GetxController {
  ...
  final rxSamplingOrderLines = <DraftTableData>[].obs;
  ...
  final rxSelectedSamplingOrderLines = <DraftTableData>[].obs;
  ...
  SamplingEntryController({required this.orderNumber}) {
    _database.managers.draftTable
        ...
        .listen(
      (draftTableRows) {
        rxSamplingOrderLines.clear();

        rxSamplingOrderLines.addAll(draftTableRows);
      },
    );
  ...
  int get _lastLineNumber => rxSamplingOrderLines.lastOrNull?.lineNumber ?? 0;
  ...
  void addLapdipOrderLine() {
    if (!validateInputs()) {
      ...
      // );
    } else if (rxSamplingOrderLines.any(
      (samplingOrderLine) =>
          samplingOrderLine.article == rxArticle.value &&
          samplingOrderLine.shade == rxShade.value,
    )) {
      toastification.show(
        autoCloseDuration: Duration(seconds: 3),
        primaryColor: VardhmanColors.red,
  ...
  void updateLapdipOrderLine() {
    final selectedSamplingOrderLine = rxSelectedSamplingOrderLines.first;
    ...
    if (!validateInputs()) {
      ...
      (samplingOrderLine) =>
          samplingOrderLine != selectedSamplingOrderLine &&
          samplingOrderLine.article == rxArticle.value &&
          samplingOrderLine.shade == rxShade.value,
    )) {
      toastification.show(
      ...
      _database.managers.draftTable
          .filter((f) => f.orderNumber.equals(orderNumber))
          .filter(
              (f) => f.lineNumber.equals(selectedSamplingOrderLine.lineNumber))
          .update(
            (o) => o(
              ...
              lineNumber: drift.Value(selectedSamplingOrderLine.lineNumber),
              orderNumber: drift.Value(selectedSamplingOrderLine.orderNumber),
              orderType: drift.Value(selectedSamplingOrderLine.orderType),
              quantity: drift.Value(selectedSamplingOrderLine.quantity),
      ...
      rxSelectedSamplingOrderLines.clear();
  ...
  void deleteSelectedLines() {
    for (DraftTableData samplingOrderLine in rxSelectedSamplingOrderLines) {
      _database.managers.draftTable
          .filter((f) => f.orderNumber.equals(orderNumber))
          .filter((f) => f.lineNumber.equals(samplingOrderLine.lineNumber))
          .delete();
    }
  ...
  Future<void> submitOrder() async {
    String? fileType;
    final submitSamplingOrderNumber = await Api.fetchSamplingOrderNumber();

    for (var samplingOrderLine in rxSamplingOrderLines) {
      if (samplingOrderLine.qtxFileName.isNotEmpty &&
          samplingOrderLine.qtxFileBytes != null) {
        final lineNumber = 1001;
        // final lineNumber = (rxDtmOrderLines.indexOf(dtmOrderLine) + 1) + 1000;

        await Api.uploadMediaAttachment(
          fileBytes: samplingOrderLine.qtxFileBytes!,
          fileName: samplingOrderLine.qtxFileName,
          moKey:
              'QTX|QT|||$submitSamplingOrderNumber|${_userController.rxUserDetail.value.soldToNumber}|$lineNumber|',
          moStructure: 'GT00092',
          version: 'VYTL0016',
          formName: 'P00092_W00092D',
        );

        fileType = samplingOrderLine.qtxFileName.split('.').lastOrNull;
      }
    }

    if (fileType != null) {
      String emails = await Api.fetchSamplingEmailAddresses();

      await Api.supplementalDataEntry(
        databaseCode: 'QTX',
        dataType: 'QT',
        orderNumber: submitSamplingOrderNumber!,
        lineNumber: 1001,
        b2bOrderNumber: 'B2BS$submitSamplingOrderNumber',
        soldToNumber: _userController.rxUserDetail.value.soldToNumber,
    ...
    final isSubmitted = await orderReviewController.submitSamplingOrder(
      b2bOrderNumber: 'B2BS$submitSamplingOrderNumber',
      merchandiserName: rxMerchandiser.value,
      samplingOrderLines: rxSamplingOrderLines,
    );
  ...
  bool get canSubmit =>
      rxMerchandiser.value.isNotEmpty && rxSamplingOrderLines.isNotEmpty;

  void selectSamplingOrderLine(DraftTableData samplingOrderLine) {
    if (rxSelectedSamplingOrderLines.contains(samplingOrderLine)) {
      rxSelectedSamplingOrderLines.remove(samplingOrderLine);
    } else {
      rxSelectedSamplingOrderLines.add(samplingOrderLine);
    }

    if (rxSelectedSamplingOrderLines.isNotEmpty) {
      clearAllInputs();

      _populateInputs(samplingOrderLine: rxSelectedSamplingOrderLines.last);
    }
  }

  void _populateInputs({required DraftTableData samplingOrderLine}) {
    final labParts = samplingOrderLine.lab.split(',');
    final isOtherBuyer = !buyerNames.contains(samplingOrderLine.buyer);
    rxMerchandiser.value = samplingOrderLine.merchandiser;
    rxBuyerName.value = isOtherBuyer ? 'OTHER' : samplingOrderLine.buyer;
    rxOtherBuyerName.value = isOtherBuyer ? samplingOrderLine.buyer : '';
    _rxBuyerCode.value = samplingOrderLine.buyerCode;
    rxFirstLightSource.value = samplingOrderLine.firstLightSource;
    rxSecondLightSource.value = samplingOrderLine.secondLightSource;
    rxArticle.value = samplingOrderLine.article;
    rxUomWithDesc.value =
        '${samplingOrderLine.uom} - ${orderReviewController.getUomDescription(samplingOrderLine.uom)}';
    rxShade.value = samplingOrderLine.shade;
    rxColor.value = samplingOrderLine.colorName;
    rxL.value = labParts[0];
    rxA.value = labParts[1];
    rxB.value = labParts[2];
    rxRequestType.value = samplingOrderLine.requestType;
    rxEndUse.value = samplingOrderLine.endUse;
    rxFileName.value = samplingOrderLine.qtxFileName;
    rxFileBytes.value = samplingOrderLine.qtxFileBytes;
    rxRemark.value = samplingOrderLine.remark;
  }
  ...
  List<DraftTableData> get samplingOrderLinesDescending =>
      rxSamplingOrderLines.toList()
        ..sort(
